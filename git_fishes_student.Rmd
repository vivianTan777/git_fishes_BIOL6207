...
set echo = FALSE, then it will not be included in the final rendered version
```
...
summary_table <-data %>%filter(!(is.na(sl))) %>% select(species, treatment, animal_id, sl, size, activity) %>%
  group_by(species, treatment) %>%
  summarise(slMean= mean(sl), sd=sd(sl), n=n(), se = sd/sqrt(n))
...
it shows the difference or the changes that have been made to this file. The red is what have been deleted and green shows the new added code.
```
...
data %>% filter(!(is.na(activity))) %>% ggplot(aes(x=activity, y=sl, colour = species)) + 
  geom_boxplot() +
...
ggsave("output/figures/fish_activity_difference.png")
```
...
The image in the output/figures/ is not showing up as uncommited changes in the changes section.
...
Yes, i think need add the .png files in the pics/ folder in GitHub, because the code is using the pictures for the plot. If not upload the pics folder, then other people who download the code will not be able to run the code successfully.
```
...
```{r, collabFig, fig.cap="Activities for chromis, lemon and acantho in different treatment", echo=TRUE, eval=TRUE}
speciesList <- c('chromis', 'lemon', 'acantho')

data %>% filter(data$species %in% speciesList) %>% filter(!(is.na(activity))) %>% ggplot(aes(x=activity, y=species, colour = species)) + 
  geom_boxplot() +
  facet_wrap(~treatment)
```
...
t_rest_dist <- rep(0, length(unique(data$species)))
t_test_results <- data.frame(matrix(ncol = 9, nrow = 0))
x <- c("species", "t", "df", "p_value", "CI_lower","CI_upper", "mean_CO2", "mean_control", "difference_means")
colnames(t_test_results) <- x
i<-0
for(spec in unique(data$species)){
  i<- i+1
  co2_activity <- filter(data, species == spec & treatment == 'CO2')
  control_activity <-filter(data, species == spec & treatment == 'control')
  t_test_val<-t.test(co2_activity$activity, control_activity$activity) 
  t_rest_dist[i] <- t_test_val
  t_test_res<-c(spec,  t_test_val$statistic, t_test_val$parameter, t_test_val$p.value, t_test_val$conf.int, t_test_val$estimate, t_test_val$null.value)
  t_test_results[i, ] <- t_test_res
   
}

```

```{r, table1, tab.cap = "activity analysis for all species", echo=TRUE, eval=TRUE, }
...
t_test_results$p_value[t_test_results$species == 'ambon']
flextable(t_test_results)
...
Mean activity for `r "ambon"` in the control group was `r t_test_results$mean_control[t_test_results$species == 'ambon']` (s / min) compared to the OA treatment group, which was `r t_test_results$mean_CO2[t_test_results$species == 'ambon']` (s / min). The difference between control and OA treatment means was `r t_test_results$difference_means[t_test_results$species == 'ambon']` (s / min) (95% CI: `r t_test_results$CI_lower[t_test_results$species == 'ambon']` to `r t_test_results$CI_upper[t_test_results$species == 'ambon']`). 
...
Null hypothesis is the activity of species `r "ambon"` for control group and the OA treatment group has no difference. Can't reject the null hypothesis because the p-value is `r t_test_results$p_value[t_test_results$species == 'ambon']`, which is very high. The t-statistic between control and OA treatment was `r t_test_results$t[t_test_results$species == 'ambon']` and the degrees of freedom for the t-statistic is `r t_test_results$df[t_test_results$species == 'ambon']`. Therefore can't reject the null hypothesis.
...
t_rest_dist <- rep(0, length(unique(data$species)))
t_test_results_permutations <- data.frame(matrix(ncol = 9, nrow = 0))
x <- c("species", "t", "df", "p_value", "CI_lower","CI_upper", "mean_CO2", "mean_control", "difference_means")
colnames(t_test_results_permutations) <- x

i<-0
for(spec in unique(data$species)){
  i<- i+1
  result_spe_CO2 <- data %>%filter(!(is.na(activity))) %>% filter( species == spec & treatment == "CO2") 
  result_spe_control <- data %>%filter(!(is.na(activity))) %>% filter(species == spec & treatment == "control") 
  mean_spe_CO2 <- mean(result_spe_CO2$activity)
  mean_spe_control <- mean(result_spe_control$activity)
  
  combined_set <- c(result_spe_CO2$activity-mean_spe_CO2,result_spe_control$activity-mean_spe_control)
  boot_combined <- combined_set[sample(1:100,replace=TRUE)]
  boot_spec_CO2 <- boot_combined[1:50]
  boot_spec_control <- boot_combined[51:100]
  tibble(boot_spec_CO2,boot_spec_control)
  
  boot_t_test_results <- t.test(boot_spec_CO2,boot_spec_control,var.equal=T,na.rm=TRUE)
  
  t_rest_dist[i] <-boot_t_test_results
  
   t_test_res<-c(spec,  boot_t_test_results$statistic, boot_t_test_results$parameter, boot_t_test_results$p.value, boot_t_test_results$conf.int, boot_t_test_results$estimate, boot_t_test_results$null.value)
  t_test_results_permutations[i, ] <- t_test_res
  
  
}

flextable(t_test_results_permutations)
...
After applying permutation, find the p-value is `r t_test_results_permutations$p_value[t_test_results_permutations$species == 'ambon']`, which is very high. The t-statistic between control and OA treatment was `r t_test_results_permutations$t[t_test_results_permutations$species == 'ambon']` and the degrees of freedom for the t-statistic is `r t_test_results_permutations$df[t_test_results_permutations$species == 'ambon']`. Therefore can't reject the null hypothesis.Therefore based on the result, we can't reject the null hypothesis.
